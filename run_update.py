...
filepath = 'src/pages/PartTimePage.jsx'
  ...
  const renderWidgetContent = (id) => {
    switch(id) {
      ...
      case 'goal':
        ...
        return (
          <div className="col-span-2 liquid-glass-card p-4 flex flex-col justify-center border-l-4 border-l-sky-500 relative">
            <div className="flex justify-between items-center mb-2">
               <p className="text-sm font-bold text-main flex items-center gap-2"><Target size={16}/> เป้าหมายเดือนนี้</p>
               <button onClick={(e) => { e.stopPropagation(); setTempGoal(incomeGoal); setShowGoalModal(true); setIsEditWidgetMode(false); }} className="text-sky-500 hover:bg-sky-500/10 p-1.5 rounded-full transition-colors z-10"><Edit size={14}/></button>
            </div>
            <p className="text-xs opacity-70 mb-2">฿{currentIncome.toLocaleString()} / ฿{incomeGoal.goalAmount.toLocaleString()}</p>
            <div className="w-full bg-main/10 rounded-full h-3 mb-2 overflow-hidden">
...
old_dashboard = """      <div className="grid grid-cols-2 gap-4 mb-8">
...
new_dashboard = """      <div className="flex justify-between items-center mb-4 px-2 mt-2">
...
modals = """      {/* Widget Selector Bottom Sheet */}
      <AnimatePresence>
        {showWidgetSelector && (
          <>
            <motion.div 
...
with open('scratch.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
